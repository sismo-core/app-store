...
import worldcoinConfigMain from "@/space-configs/main/worldcoin";
    ...
    <CustomAppContainer>
      {!finished && (
        <>
          <Section
            number={1}
            isOpen={!sismoResponse}
            title="Sign in with Sismo"
            style={{ marginBottom: 16 }}
            success={Boolean(sismoResponse)}
          >
            <ProveEligibility app={app} onEligible={(_response) => setSismoResponse(_response)} />
          </Section>
          <Section
            number={2}
            isOpen={Boolean(sismoResponse)}
            title={app?.templateConfig?.step2CtaText}
          ...
          >
            {alreadySubscribed ? (
              ...
              <Verifications>
                ...
                >
                  {({ open }) => (
                    <Button onClick={open} loading={verifyingOrb} style={{ width: "100%" }}>
            ...
            <Button3D
              onClick={() =>
                redirect
                  ...
                  : router.push(`/${worldcoinConfigMain.metadata.slug}`)
              }
              secondary
            >
