...
export async function POST(req: Request) {
  ...
  if (env.isDev) {
    console.info(`${request.username} (${request.userId}) requests to join ${request.groupTitle} (${request.groupId})`);
  }
  ...
  try {
    ...
    if (env.isDev) { console.error(error.message); }
...
const isWhitelistApproved = async (app: ZkTelegramBotAppConfig, telegramId: string): Promise<boolean> => {
...
const approvedResponse = (approved: boolean): Response => {
  if (env.isDev) { console.info(`Approved: ${approved}`); }
